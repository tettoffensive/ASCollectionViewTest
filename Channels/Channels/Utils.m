...
+ (BOOL)deviceUsesLowResolutionCamera {
    ...
    NSString *device = [Utils deviceIdentifier];
...
+ (NSString *)formatLargeNumber:(int)number {
    ...
    if (number < 10000) {
        ...
        float n = number / 1000000.0f;
        numberString = [NSString stringWithFormat:@"%0.1fM",n];
    } else {
        float n = number / 1000000;
...
+ (NSString *)imageProcessedForUpload:(UIImage *)image withCompletion:(void (^)(NSData *, NSString *))complete {
    ...
    __block NSString *identifier = [Utils randomIdentifierWithLength: 8];
...
void __SPDebug(const char *fileName, int lineNumber, NSString *fmt, ...)
{
    ...
    __SPDebugPrint(fileName, lineNumber, logMsg);
    va_end(args);
#endif
}
...
double screenWidth()
{
    return (NO ? [UIScreen mainScreen].bounds.size.height : [UIScreen mainScreen].bounds.size.width);
}

double screenHeight()
{
    return (NO ? [UIScreen mainScreen].bounds.size.width : [UIScreen mainScreen].bounds.size.height + 1.0f);
}
...
CGSize resizeWidthWhilePreservingAspectRatio(CGSize currentSize, CGFloat newSize)
{
    ...
    float ratio     = (newSize/currentSize.width);
...
{
    CGSize size     = CGSizeZero;
    float ratio     = (newSize/currentSize.height);
    size.width      = currentSize.width*ratio;
    size.height     = currentSize.height*ratio;
