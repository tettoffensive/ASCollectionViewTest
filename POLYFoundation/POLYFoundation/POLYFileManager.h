...
@property (nonatomic, strong) NSString *baseURL; // @"http://cdn.joinswipe.com"
...
- (void)uploadImage:(UIImage *)image progress:(void (^)(CGFloat progress))progressBlock success:(void (^)(BOOL finished, NSString *key))successBlock failure:(void (^)(NSError *err))failureBlock;
- (void)uploadVideoData:(NSData *)data progress:(void (^)(CGFloat progress))progressBlock success:(void (^)(BOOL finished, NSString *key))successBlock failure:(void (^)(NSError *err))failureBlock;
